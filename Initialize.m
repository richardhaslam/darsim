function [Status] = Initialize(Fluid,Grid,FlashSettings)
...
if (strcmp(Fluid.Type,'Immiscible')==1)         %For the immiscible displacement case
    ...
    Status.x1 = zeros(Grid.N, 2, 1);            %Defines phase mole fraction for 2 phase 2 component (each phase has its own column, values stored are for component 1)
...
elseif (strcmp(Fluid.Type,'BlackOil')==1)       %For the black oil model
...
elseif (strcmp(Fluid.Type,'Compositional')==1)  %For the compositional case
