...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
classdef builder_FracGen < handle
    properties
        type
        dim
        size
    ...
    methods
        function FindKeyWords(obj, inputMatrix)
            ...
            temp = strfind(inputMatrix{1}, 'TYPE');
            obj.type = find(~cellfun('isempty', temp));
        ...
        function BuildFracInput(obj, inputMatrix)
            ...
            for f = 1 : obj.NrOfAllFracs
                ...
                obj.FracInput(f).RotAngleAlongL = str2double( frac_temp{06} );
                if strcmp(inputMatrix{1}{obj.dim+1},'2D'),  obj.FracInput(f).RotAngleAlongL = 90;   end
                %%% Rotation angle along central length %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                obj.FracInput(f).RotAngleAlongW = str2double( frac_temp{07} );
                if strcmp(inputMatrix{1}{obj.dim+1},'2D'),  obj.FracInput(f).RotAngleAlongW = 0;    end
        ...
        function Geometry = BuildGeometry(obj, inputMatrix)
            Geometry = geometry_FracGen();
            Geometry.Type = inputMatrix{1}{obj.type+1};
            Geometry.Domain = inputMatrix{1}{obj.dim+1};
            Geometry.ReservoirGrid = obj.BuildReservoir(inputMatrix);
            Geometry.FracturesGrid = obj.BuildFractures(Geometry.ReservoirGrid);
        end
        function Reservoir = BuildReservoir(obj, inputMatrix)
            ...
            if strcmp(inputMatrix{1}{obj.dim+1},'2D')
                Reservoir.NZ = 1;
            end
            CompareAccuracy = str2double( inputMatrix{1}{obj.compareAccuracy+1} );
            Type = inputMatrix{1}{obj.type+1};
            Reservoir.Discretize(CompareAccuracy,Type);
        end
        function FracturesGrid = BuildFractures(obj, ReservoirGrid)
            FracturesGrid = fracturesGrid_FracGen(ReservoirGrid.Epsilon);
            FracturesGrid.BuildFracturesGrid(ReservoirGrid, obj.FracInput);
        end
    end
end