                ...
                for j=1:length(b)
                    for c=1:obj.NofComponents
                        ...
                        Jp{c}(b(j),b(j)) = Jp{c}(b(j),b(j))...
                            + Prod(i).PI * K(b(j)) *...
                            ( obj.Mob(b(j), 1) * State.rho(b(j),1) * x(b(j), (c-1)*2+1) ...
                            + obj.Mob(b(j), 2) * rho(b(j),2) * x(b(j), (c-1)*2+2)...
                            )...
                            - Prod(i).PI * K(b(j)) * obj.Mob(b(j), 1) * x(b(j), (c-1)*2+1) * obj.drhodp(b(j),1) * (Prod(i).p(j) - State.p(b(j))) ...
                            - Prod(i).PI * K(b(j)) * obj.Mob(b(j), 1) * obj.dxdp(b(j), (c-1)*2+1) * rho(b(j),1) * (Prod(i).p(j) - State.p(b(j))) ...
                            - Prod(i).PI * K(b(j)) * obj.Mob(b(j), 2) * x(b(j), (c-1)*2+2) * obj.drhodp(b(j),2) * (Prod(i).p(j) - State.p(b(j)))...
                            - Prod(i).PI * K(b(j)) * obj.Mob(b(j), 2) * obj.dxdp(b(j), (c-1)*2+2) * rho(b(j),2) * (Prod(i).p(j) - State.p(b(j)));
