    ...
    methods
        ...
        function Jacobian = BuildJacobian(obj, ProductionSystem, DiscretizationModel, dt)
            ...
            for i=1:obj.NofComponents
                ...
                dMupxPh2 = obj.UpWind(2).x * (obj.Mob(:, 2) .* x(:,(i-1)*2+2) .* obj.drhodp(:,2) + obj.Mob(:, 1) .* obj.dxdp(:,(i-1)*2+2) .* rho(:,2));
                dMupyPh2 = obj.UpWind(2).y * (obj.Mob(:, 2) .* x(:,(i-1)*2+2) .* obj.drhodp(:,2) + obj.Mob(:, 1) .* obj.dxdp(:,(i-1)*2+2) .* rho(:,2));
            ...
            for i=1:Wells.NofProd
                ...
                for j=1:length(b)
                    ...
                    Jp{2}(b(j),b(j)) = Jp{2}(b(j),b(j)) ...
                        ...
                        - Prod(i).PI * K(b(j)) * obj.Mob(b(j), 1) * x(b(j),3) * obj.drhodp(b(j), 2) * (Prod(i).p - State.p(b(j))) ...
