        ...
        function [Jp, JS] = BuildMediumJacobian(obj, Medium, Wells, Grid, dt, Index, f, ph)
            ...
            switch class(Grid)
                case('corner_point_grid')
                    ...
                    dMobUp = Grid.ConnectivityMatrix' * ( obj.Mob(Index.Start:Index.End, ph) .* obj.drhodp(Index.Start:Index.End, ph) );
                    dMobUp = obj.UpWind{ph,1+f} .* min(obj.U{ph,1+f},0) .* dMobUp;
                    acc = pv/dt .* obj.drhodp(Index.Start:Index.End,ph) .* s;
                    Jp = Jp + spdiags(acc,0,N,N) + Grid.ConnectivityMatrix * spdiags(dMobUp ,0,N_Face,N_Face) * Grid.ConnectivityMatrix';
                    
                    % 2. Saturation Block
                    dMobUp = Grid.ConnectivityMatrix' * ( obj.dMob(Index.Start:Index.End, ph) .* rho );
                    dMobUp = obj.UpWind{ph,1+f} .* min(obj.U{ph,1+f},0) .* dMobUp;
                    v = (-1)^(ph+1) .* pv/dt .* rho;
                    JS = spdiags(v,0,N,N) + Grid.ConnectivityMatrix * spdiags(dMobUp ,0,N_Face,N_Face) * Grid.ConnectivityMatrix';
