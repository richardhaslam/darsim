...
for i=1:length(inj)
    ...
    %Inj(i).PI = 2000;
    Inj(i).PI = ComputeProductivityIndex(Inj.r, K(1, 1, 54), K(2, 1, 54), Grid.dx, Grid.dy, 1);
end
%Production Wells
for i=1:length(prod)
    ...
    %Prod(i).PI = 2000;
    Prod(i).PI = ComputeProductivityIndex(Prod.r,K(1, 216, 1), K(2, 216, 1), Grid.dx, Grid.dy, 1);
end
end