...
Grid = ComputeTransmissibility(Grid, Kt);

%Construct pressure matrix
A = AssemblePressureMatrix(Grid);
Ab = A;
q = zeros(N,1);
...
U.x = zeros(Nx+1,Ny,1);
U.y = zeros(Nx,Ny+1,1);
U.x(2:Nx,:) = (P(1:Nx-1,:)-P(2:Nx,:)).*Grid.Tx(2:Nx,:) - Ucap.x(2:Nx,:);
U.y(:,2:Ny) = (P(:,1:Ny-1)-P(:,2:Ny)).*Grid.Ty(:,2:Ny) - Ucap.y(:,2:Ny);
