...
classdef cube_DARSim < hexahedron_DARSim
    ...
    methods
        ...
        %%
        function [intersectCoordFinal, areCoplanar] = Obtain_Cube_Tetragon_Intersection(obj, Tetragon, Epsilon)
            ...
            for nr = 1 : size( intersectCoordTemp , 1 )
                ...
                if ( ( XimCube(1) - intersectCoordTemp(nr,1) ) > Epsilon ) || ( ( intersectCoordTemp(nr,1) - XimCube(2) ) > Epsilon )
                    intersectCoordTemp(nr,:) = [ NaN , NaN , NaN ];
                    continue;
                end
                if ( ( YimCube(1) - intersectCoordTemp(nr,2) ) > Epsilon ) || ( ( intersectCoordTemp(nr,2) - YimCube(2) ) > Epsilon )
                    intersectCoordTemp(nr,:) = [ NaN , NaN , NaN ];
                    continue;
                end
                if ( ( ZimCube(1) - intersectCoordTemp(nr,3) ) > Epsilon ) || ( ( intersectCoordTemp(nr,3) - ZimCube(2) ) > Epsilon )
                    intersectCoordTemp(nr,:) = [ NaN , NaN , NaN ];
                    continue;
                end
