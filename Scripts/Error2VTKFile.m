...
function Error2VTKFile(Case1, Case2, ComparisonDir, n_files)
%Read the input file to acquire info about geometry etc.
InputFile = strcat(Case1.Dir, Case1.File);
...
%Read the input file to acquire info about geometry etc.
InputFile = strcat(Case2.Dir, Case2.File);
fileID = fopen(InputFile, 'r');
%// Read lines from input file
inputMatrix = textscan(fileID, '%s', 'Delimiter', '\n');
fclose(fileID);
...
for i=0:Nstops
file = strcat(Case1.Dir, 'Output/Solution/',Problem1,'_Sol', num2str(i),'.txt');
Sol_1 = load(file);

file = strcat(Case2.Dir, 'Output/Solution/',Problem2,'_Sol', num2str(i),'.txt');
Sol_2 = load(file);
    ...
    %Saturation error
    SaturationError = abs(Sol_2(:,3) - Sol_1(:,3));
