...
for n = 1:N_Comparison
    File = strcat(Directory, 'ImmHomo_ADM_', num2str(tol(n)),'_ADMStats.txt');
    FileID = fopen(File, 'r');
...
%% Plot of Average Pressure Error, Temperature Error, Rock Temperature Error and Active Grid Cells versus Different Tolerances
Error_P_Ave = zeros(1,length(tol));
Error_T_Ave = zeros(1,length(tol));
Error_Tr_Ave(n) = mean(Error_T(:,n));
ADMStats_Ave = zeros(1,length(tol));
for n = 1 : length(tol)
    Error_P_Ave(n) = mean(Error_P(:,n));
    Error_T_Ave(n) = mean(Error_T(:,n));
    Error_Tr_Ave(n) = mean(Error_Tr(:,n));
...
yyaxis right; plot( tol , Error_T_Ave , '--*b' , 'LineWidth',2 , 'MarkerSize',12 );
yyaxis right; plot( tol , Error_Tr_Ave , '--+k' , 'LineWidth',2 , 'MarkerSize',12 );
...
legend({'Average Perssure Error (left axis)', 'Average Temperature Error (right axis)', 'Average Rock Temperature Error (right axis)', 'Avergare Active Grid Cells [-] (right axis)'},'FontSize', 15);
