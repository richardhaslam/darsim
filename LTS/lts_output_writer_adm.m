classdef lts_output_writer_adm < output_writer
    properties
        FormatADM
        basisfunctions
        dynamicBF
    ...
    methods
        function obj = lts_output_writer_adm(dir, problem, n_inj, n_prod, n_timers,  n_stats, n_comp)
        ...
        function WriteSummary(obj, Summary)
            ...
            obj.WriteLTSIters(Summary.LTSIter, Summary.NumberTimeSteps);
        end
        ...
        function WriteLTSIters(obj, LTSIter, Ndt)
            % Stats
            fileID = fopen(strcat(obj.Directory,'LTSIter.txt'),'w');
            [~, nc] = size(LTSIter);
            format = '%8d';
            for i =1:nc-2
                format = [format, ' %8d'];
            ...
            format = [format, ' %8d\n'];
            fprintf(fileID, format, LTSIter(1:Ndt, :)');
            fclose(fileID);
        end
    end
end
